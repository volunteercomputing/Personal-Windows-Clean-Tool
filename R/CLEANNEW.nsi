...
/*  !define MUI_COMPONENTSPAGE_SMALLDESC*/
  ...
  Pop $R0
  StrCmp $R0 0 +3
    MessageBox MB_OK|MB_ICONEXCLAMATION "另一个程序正在运行中，请您稍后再拨！！"
    Abort
  FunctionEnd
...
SectionEnd
Section "!最近的打开文档历史记录"  RECENTCLEAN
  Delete $RECENT\*.*
  RMDir /r "$RECENT"
...
Section /o "Windows 画图及写字板历史记录"  PAINTCLEAN
  ...
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Applets\Wordpad\Recent File List"
SectionEnd
Section /o "Windows 预读、日志、内存转储、开始菜单等文件"  MEMCLEAN
...
Section "Office 软件打开历史记录"  OFFICESOFTCLEAN
...
SectionGroupEnd
  ...
  LangString   		DESC_RECENTCLEAN					${LANG_SimpChinese}				"清理系统中的文件、文件夹、历史记录。"
  LangString   		DESC_PAINTCLEAN						${LANG_SimpChinese}				"清理系统自带的画图及写字板软件中的历史记录。"
  LangString  		DESC_MEMCLEAN						${LANG_SimpChinese}				"清理系统自动保存的预读、内存转储、系统日志、开始菜单等文件及历史内容。定期清理可释放大量磁盘空间！"
  ...
  BrandingText																			"原创开发制作：周潇波 Copyright (C) 2009-2012 By 周潇波";EOF NOW